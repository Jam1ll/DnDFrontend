...
import {
  type SubRaceResponseDTO,
  useSubRace,
} from "../../../../api/manual/race/subRace/subRaceService";

interface EditSubRaceProps extends SubRaceResponseDTO {
  raceId: string;
}

export const EditSubRaceButton = ({
  name,
  description,
  id,
  manualId,
  ...
  const updateSubRace = useSubRace.useUpdate();
  ...
  return (
    <>
      <button
        ...
        className="cursor-pointer p-4 bg-gray-700/50 hover:bg-blue-600 text-gray-300 hover:text-white rounded-md transition-all duration-200 ml-auto"
      >
        <svg
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        <form onSubmit={handleSubmit} className="max-w-sm mx-auto space-y-4">
          ...
          <div className="flex justify-end pt-4">
            <button
              type="submit"
              disabled={updateSubRace.isPending}
            ...
            >
              <span className="text-sm tracking-wide font-medium">
                {updateSubRace.isPending ? "GUARDANDO..." : "GUARDAR"}
              </span>
            </button>
          </div>
        </form>
