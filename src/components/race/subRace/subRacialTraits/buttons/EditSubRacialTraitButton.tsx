...
import {
  useSubRacialTrait,
  type SubRacialTraitResponseDTO,
} from "../../../../../api/manual/race/subRace/subRacialTrait/subRacialTraitService";
import { Modal } from "../../../../ui/Modal";

interface EditSubRacialTraitProps extends SubRacialTraitResponseDTO {
  subRaceId: string;
}

export const EditSubRacialTraitButton = ({
  name,
  description,
  id,
  manualId,
  subRaceId,
}: EditSubRacialTraitProps) => {
  const queryClient = useQueryClient();
  const [isOpen, setIsOpen] = useState(false);
  const [newName, setNewName] = useState(name);
  const [newDescription, setNewDescription] = useState(description);

  const updateSubRacialTrait = useSubRacialTrait.useUpdate();
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
        <form onSubmit={handleSubmit} className="max-w-sm mx-auto space-y-4">
          ...
          <div className="flex justify-end pt-4">
            <button
              type="submit"
              disabled={updateSubRacialTrait.isPending}
            ...
            >
              <span className="text-sm tracking-wide font-medium">
                {updateSubRacialTrait.isPending ? "GUARDANDO..." : "GUARDAR"}
              </span>
            </button>
          </div>
        </form>
